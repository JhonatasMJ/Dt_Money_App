...
export function TransactionCard({ amount, type }: Props) {
      ...
      return (
        <Text className="text-white text-base">
          {filters.from && filters.to
            ? `${format(filters.from, "dd/MM/yyyy")} a ${format(filters.to, "dd/MM/yyyy")}`
  ...
  return (
    <View
      className={clsx(`bg-${cardData.bgcolor} min-w-[280] rounded-md px-8 py-6 justify-between mr-6 mt-4`, type === "total" && "mr-12")}
    >
